...
sub authorize {
    ...
    if(!exists($RAD_REQUEST{'EAP-Message'})){
        if($RAD_CHECK{'Rd-Avail-Bytes'} >= $int_max){
            ...
            $RAD_REPLY{'Ls-Total-Limit'}		= $RAD_CHECK{'Rd-Avail-Bytes'} % $int_max;
            $RAD_REPLY{'Ls-Total-Limit-Gigawords'}	= int($RAD_CHECK{'Rd-Avail-Bytes'} / $int_max);

        }else{
            $RAD_REPLY{'Ls-Total-Limit'}		= $RAD_CHECK{'Rd-Avail-Bytes'};
        }
    }
