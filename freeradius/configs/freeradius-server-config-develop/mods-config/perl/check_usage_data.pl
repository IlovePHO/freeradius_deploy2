...
sub authorize {
    ...
    if($RAD_CHECK{'Rd-Avail-Data'} <= 0){
        if($RAD_CHECK{'Rd-Reset-Type-Data'} ne 'never'){
            $RAD_REPLY{'Reply-Message'} = "Sorry... you have used your  $RAD_CHECK{'Rd-Reset-Type-Data'} data allowance";
        }else{
            $RAD_REPLY{'Reply-Message'} = "Sorry... you have used all your data allowance ";
        }
    ...
    if(!exists($RAD_REQUEST{'EAP-Message'})){
        if($RAD_CHECK{'Rd-Avail-Data'} >= $int_max){
            #Lifeseed's internal reply attributes
            $RAD_REPLY{'Ls-Total-Limit'}		= $RAD_CHECK{'Rd-Avail-Data'} % $int_max;
            $RAD_REPLY{'Ls-Total-Limit-Gigawords'}	= int($RAD_CHECK{'Rd-Avail-Data'} / $int_max);

        }else{
            $RAD_REPLY{'Ls-Total-Limit'}		= $RAD_CHECK{'Rd-Avail-Data'};
        }
    }
