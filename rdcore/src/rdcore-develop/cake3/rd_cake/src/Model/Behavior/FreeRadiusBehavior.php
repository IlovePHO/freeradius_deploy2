...
use phpseclib\mcrypt_compat\lib\mcrypt;
...
class FreeRadiusBehavior extends Behavior {
    ...
    protected $convertAttributes = [
        'Cleartext-Password' => [
            [
                'attribute' => 'Ls-AES-NT-Password',
                'method'    => '_convertLsAesNtPassword',
            ],
        ],
    ];
         ...
         foreach(array_keys($this->puChecks) as $key){
            if($entity->dirty("$key")){
                ...
                if($value !== null){
                    ...
                    $this->_convert_replace_radcheck_item($username,$this->puChecks["$key"],$value);
                }
            }  
        }
    ...
    private function _forVoucherAdd($entity){
        ...
        foreach(array_keys($this->vChecks) as $key){
            if(($entity->{$key} !== '')&&($entity->{$key} !== null)){
                ...
                $this->_convert_add_radcheck_item($username,$this->vChecks["$key"],$value);
            }
        }
    ...
    private function _forDeviceAdd($entity){
        ...
        foreach(array_keys($this->puChecks) as $key){
            if(($entity->{$key} !== '')&&($entity->{$key} !== null)){
                ...
                $this->_convert_add_radcheck_item($username,$this->puChecks["$key"],$value);
            }
        }
    ...
    private function _convert_add_radcheck_item($username, $item, $value, $op = ":=") {
        if (array_key_exists($item, $this->convertAttributes)) {
            $convert_attributes = $this->convertAttributes[$item];
            foreach ($convert_attributes as $convert_attribute) {
                $attribute = $convert_attribute["attribute"];
                $method = $convert_attribute["method"];

                if (is_callable([$this, $method])) {
                    $convert_value = call_user_func([$this, $method], $value);
                    $this->_add_radcheck_item($username, $attribute, $convert_value, $op);
                }
            }
        } else {
            $this->_add_radcheck_item($username, $item, $value, $op);
        }
    }

    private function _convert_replace_radcheck_item($username, $item, $value, $op = ":=") {
        if (array_key_exists($item, $this->convertAttributes)) {
            $convert_attributes = $this->convertAttributes[$item];
            foreach ($convert_attributes as $convert_attribute) {
                $attribute = $convert_attribute["attribute"];
                ...
                if (is_callable([$this, $method])) {
                    $convert_value = call_user_func([$this, $method], $value);
                    $this->_replace_radcheck_item($username, $attribute, $convert_value, $op);
                }
            }

            //Remove origial attribute
            $this->_remove_radcheck_item($username, $item);
        } else {
            $this->_replace_radcheck_item($username, $item, $value, $op);
        }
    }
    ...
    private function _keyHash($key, $key_size) {
        $key = md5($key, true);
    ...
    private function _convertLsAesNtPassword($password){
        $password = iconv('UTF-8', 'UTF-16LE', $password);
        $md4_hash_hex = hash('md4', $password);
        $md4_hash_bin = hex2bin($md4_hash_hex);
        ...
        $padding_data = $this->_pkcs5Pad($md4_hash_bin, $block_size);
        $key = $this->_keyHash($key, $key_size);
        $aes_encrypted = mcrypt_encrypt(MCRYPT_RIJNDAEL_128, $key, $padding_data,
                                        MCRYPT_MODE_CBC, $iv);
        $base64_encoded = base64_encode($aes_encrypted);
        $encrypted = sprintf("%s:%s", $iv, $base64_encoded);
