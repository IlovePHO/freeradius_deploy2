...
class NasController extends AppController{
	...
	public function uploadPhoto($id = null){
        ...
        $unique         = time();
        $dest           = WWW_ROOT."img/nas/".$unique.'.'.$path_parts['extension']; // WWW_ROOT <- IMAGES
        ...
        $this->set(array(
            'data'   => $json_return, //For the form to load we use data instead of the standard items as for grids
            'success' => true,
            '_serialize' => array('success','data')
        ));
    }
    ...
    //public function edit_map_pref(){
        ...
        if(array_key_exists('zoom',$cdata)){
			...
			$d['name']   = 'map_zoom';
			$d['value']  = $cdata['zoom'];
			$this->UserSettings->deleteAll(['user_id' => $user_id,'name' => $d['name']]);
    ...
    //public function delete_map(){
        ...
        if(isset($cdata['id'])){
			$modelEntity = $this->{$this->main_model}->newEntity($cdata);
            $modelEntity->lat = null;
            $modelEntity->lon = null;
			if (!$this->{$this->main_model}->save($modelEntity)) {
