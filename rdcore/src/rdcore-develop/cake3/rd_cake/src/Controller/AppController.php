...
class AppController extends Controller
{
    ...
    public function initialize()
    {
        ...
        // Set to the login user's language
        $session  = $this->getRequest()->getSession();
        if(!empty($this->request->getData('language_code'))){
            $session->write('Config.language',$this->request->getData('language_code'));
        }elseif(!empty($this->request->getQuery('language_code'))){
            $session->write('Config.language',$this->request->getQuery('language_code'));
        }else{
          // sel_language = {country_id}_{language_id}
          $sel_language = null;
          if(!empty($this->request->getData('sel_language'))){
              $sel_language = $this->request->getData('sel_language');
          }elseif(!empty($this->request->getQuery('sel_language'))){
              $sel_language = $this->request->getQuery('sel_language');
          }
          if(!empty($sel_language)){
            $country_language   = explode( '_', $sel_language );
            //$country            = $country_language[0];
            //$language           = $country_language[1];
            // language_id to iso-code
            $languages = TableRegistry::get('languages');
            $language = $languages->find()->where(['id' => $country_language[1]])->first();
            if(!empty($language)){
              $session->write('Config.language',$language->iso_code);
            }
          }
        }
        $language_code =$session->read('Config.language');
