...
class ProfilesController extends AppController
{
	...
	public function simpleAdd(){
        ...
        if ($this->request->data['vlan_enabled']) {
            $vlan_id = $this->request->data('vlan_id');
            if (is_null($vlan_id) || !is_numeric($vlan_id)) {
	...
	public function simpleView(){
        ...
        $data = ['speed_limit_enabled' =>false,'time_limit_enabled' =>false,
                 'data_limit_enabled' => false,'vlan_enabled' => false];
    ...
    private function _doRadius($groupname){
        ...
        if($this->request->data['speed_limit_enabled']){ //IF it is there    
         ...
         if($this->request->data['vlan_enabled']){ //IF it is there    
            $vlan_id = $this->request->data['vlan_id'];

            $d_list = [
                [
                    'groupname' => $groupname,
                    'attribute' => 'Ls-Vlan-Id',
                    'op'        => ':=',
                    'value'     => $vlan_id,
                    'comment'   => 'SimpleProfile'
                ],
                [
                    'groupname' => $groupname,
                    'attribute' => 'Ls-Vlan-Id-Type',
                    'op'        => ':=',
                    'value'     => 0, // 0: 802.1q, 1: 802.1ad
                    'comment'   => 'SimpleProfile'
                ],
            ];
 
            foreach ($d_list as $d_e) {
                $entity = $this->{'Radgroupreplies'}->newEntity($d_e);
                $this->{'Radgroupreplies'}->save($entity);
            }
        }
