...
class PermanentUsersController extends AppController{
    ...
    public function exportCsv(){
        ...
        if($this->CommonQuery->build_with_realm_query($query,$user,['Users','PermanentUserNotes' => ['Notes'],'Realms'],'username','PermanentUsers') == false){
    ...
    public function index(){
        ...
        foreach($q_r as $i){
            ...
            if (isset($i->{"sub_group"})) {
                $row['sub_group_name'] = $i->{'sub_group'}->{'name'};
            } else {
                $row['sub_group_name'] = '';
            }
    ...
    public function add(){
       ...
       */
        ...
        //---Set sub_group related things---
        $sub_group_id = $this->request->data('sub_group_id');
        if (!empty($sub_group_id) && $sub_group_id > 0) {
            $sub_group_entity = $this->SubGroups->get($sub_group_id);
            if (!$sub_group_entity) {
                $this->JsonErrors->errorMessage('sub_group_id not found in DB or not supplied');
                return;
            }
        } elseif (!is_null($sub_group_id)) {
            $this->request->data['sub_group_id'] = null;
        }
        ...
        $voucher = $this->_findVoucherByUsername($this->request->data['username']);
        if (!is_null($voucher)) {
            $this->set(array(
                'success' => false,
                'errors' => ['username' => 'The username you provided is already taken. Please provide another one.' ],
