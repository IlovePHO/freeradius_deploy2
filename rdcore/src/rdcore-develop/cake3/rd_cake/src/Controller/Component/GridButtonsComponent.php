...
class GridButtonsComponent extends Component {
    ...
    public function returnButtons($user,$title = true,$type='basic'){
        ...
        if($type == 'realms'){
            ...
            $p  = $this->_fetchRealmProxy();
            $s  = $this->_fetchSubGroups();
            $menu = array($b,$d,$a,$p,$s);
        }
        if($type == 'Idps'){
            $b  = $this->_fetchBasic('disabled',true);
            $a  = $this->_fetchIdpsExtras();
            //$s  = $this->_fetchSubGroups();
            $menu = [ $b, $a];
        }
        if($type == 'ExternalApiKeys'){
            $b  = $this->_fetchBasic('disabled',true);
            $menu = array($b);
        }
        if($type == 'EncodingSchemes'){
            $b  = $this->_fetchBasic('disabled',true);
            $menu = array($b);
        }
        if($type == 'StaConfigs'){
        ...
        if($type == 'SubGroups'){
            $b  = $this->_fetchBasicSubGroups();
            $a  = $this->_fetchSubGroupsExtras();
            $menu = [ $b, $a];
        }
        ...
        if($type == 'StaInfos'){
            $b  = $this->_fetchBasicStaInfos('disabled',false);
            $menu = array($b);
        }
        ...
        if($type == 'Proxies'){
            $b  = $this->_fetchBasic('disabled',true,$type);
            $a  = $this->_fetchExtrasProxies();
            $menu = [ $b, $a ];
        }
        if ( $type == 'ProxyDecisionConditions' ) {
            $b  = $this->_fetchBasic('disabled',true,$type);
            $menu = [ $b ];
        }
        if ( $type == 'AttributeConverts' ) {
            $b = $this->_fetchBasicAttributeConverts();
            $a  = $this->_fetchAddAttributeConverts(__('Selection'));
            $menu = [ $b, $a ];
        }
        if ( $type == 'HomeServerPools' ) {
            $b  = $this->_fetchBasic('disabled',true,$type);
            $a  = $this->_fetchExtrasHomeServerPools();
            $menu = [ $b, $a ];
        }
        if ( $type == 'HomeServers' ) {
            $b  = $this->_fetchBasic('disabled',true,$type);
        ...
        //VPN
        if($type == 'SoftEtherVpnInstances'){
            $b  = $this->_fetchBasic('disabled',true);
            $a  = $this->_fetchSoftEtherVpnInstancesExtras();
            $menu = [ $b, $a];
        }
        if($type == 'SoftEtherVirtualHubs'){
            $b  = $this->_fetchBasic('disabled',true);
            $menu = [ $b ];
        }
        if($type == 'SoftEtherUsers'){
            $b  = $this->_fetchBasic('disabled',true);
            $menu = [ $b ];
        }
        if($type == 'SoftEtherLocalBridges'){
            $b  = $this->_fetchhBasicSoftEtherLocalBridges('disabled',false);
            $menu = [ $b ];
        }
        if($type == 'SoftEtherWireguardPublicKeys'){
            $b  = $this->_fetchhBasicSoftEtherWireguardPublicKeys('disabled',false);
            $menu = [ $b ];
        }
        if($type == 'SoftEtherNetworkBridges'){
            $b  = $this->_fetchBasic('disabled',true);
            $menu = [ $b ];
        }
        if($type == 'SoftEtherInterfaces'){
            $b  = $this->_fetchhBasicSoftEtherInterfaces('disabled',false);
            $menu = [ $b ];
        }
        return $menu;
    }
    ...
    private function _fetchDynamicTranslations(){
        ...
        if($user['group_name'] == Configure::read('group.ap') || //AP (with overrides)
           $user['group_name'] == Configure::read('group.sm')){  //SM (with overrides)
            $cmb_options = [
                'xtype'     => 'cmbDynamicDetailTransOptions',
                'margin'    => '5 0 5 0',
                'isRoot'    => false,
     ...
     private function _fetchBasicMeshNodes(){
       ...
       $menu        = [];
        ...
        if($user['group_name'] == Configure::read('group.ap') || //AP
           $user['group_name'] == Configure::read('group.sm')){  //SM
            $id             = $user['id'];
            $action_group   = [];
            $disabled       = true;
            $action_group   = [
    ...
    private function _fetchNas(){
        ...
        if($user['group_name'] == Configure::read('group.ap') || //AP (with overrides)
           $user['group_name'] == Configure::read('group.sm')){  //SM (with overrides)
            ...
            //Tags
            if($this->Acl->check(array('model' => 'Users', 'foreign_key' => $id), $this->controller->base.'manageTags')){
    ...
    private function _fetchRealmProxy(){
        $user = $this->user;
            ...
            $menu = array(
                ...
                'items' => array(
                    ...
                    array(
                        ...
                        'glyph'     => Configure::read('icnGears'),
                        'scale'     => $this->scale,
                        'itemId'    => 'ProxyOperation',
                        'tooltip'   => __('Proxy operation'),
                        'ui'        => $this->btnUiAdvancedEdit
                    )
                )
            );
        ...
        if($user['group_name'] == Configure::read('group.ap') || //AP (with overrides)
           $user['group_name'] == Configure::read('group.sm')){  //SM (with overrides)
            ...
            if($this->Acl->check(array('model' => 'Users', 'foreign_key' => $id), 'Access Providers/Controllers/Proxies/index')){
                array_push($m_item, array(
                        'xtype'     => 'button',
                        'glyph'     => Configure::read('icnComponent'),
                        'scale'     => $this->scale,
                        'itemId'    => 'Proxies',
           ...
           if($this->Acl->check(array('model' => 'Users', 'foreign_key' => $id), 'Access Providers/Controllers/proxiesRealms/add') ||
              $this->Acl->check(array('model' => 'Users', 'foreign_key' => $id), 'Access Providers/Controllers/proxiesRealms/delete')){
                array_push($m_item, array(
                        'xtype'     => 'button',
                        'glyph'     => Configure::read('icnGears'),
                        'scale'     => $this->scale,
            ...
            $menu = array('xtype' => 'buttongroup', 'title' => $t, 'items' => $m_item );
        }
    ...
    private function _fetchExtrasProxies(){
        $user = $this->user;
        ...
        $hsp_btn = [
                        ...
                        'glyph'     => Configure::read('icnComponent'),
                        'scale'     => $this->scale,
                        'itemId'    => 'home_server_pools',
                        'tooltip'   => __('home server pools'),
                        'ui'        => $this->btnUiProfComp
                    ];

        //Admin => all power
        if($user['group_name'] == Configure::read('group.admin')){  //Admin
            array_push($m_items,$hsp_btn);
        }
        if($user['group_name'] == Configure::read('group.ap') || //AP (with overrides)
           $user['group_name'] == Configure::read('group.sm')){  //SM (with overrides)
            $id  = $user['id'];
            if($this->controller->Acl->check(['model' => 'Users', 'foreign_key' => $id], 'Access Providers/Controllers/HomeServerPools/index')){
                array_push($m_items,$hsp_btn);
            }
        }
        if( !empty($m_items) ){
    ...
    private function _fetchBasicAttributeConverts( $action = 'disabled', $with_reload_timer = false, $type = '' ) {
        $user = $this->user;
        ...
        if ( $user[ 'group_name' ] == Configure::read( 'group.ap' ) ) { //AP (with overrides)
            ...
            array_push( $m_items, $reload );

            //Delete
            if ( $this->controller->Acl->check( array( 'model' => 'Users', 'foreign_key' => $id ), $this->controller->base . 'delete' ) ) {
                array_push( $m_items, $this->btnDelete );
            }

        }
        if( !empty($m_items) ){
            $menu = [
                        'xtype' => 'buttongroup', 'title' => $this->t,
                        'items' => $m_items,
                    ];
        }
        return $menu;
    ...
    private function _fetchAddAttributeConverts( $title='' ) {
        $user = $this->user;
        ...
        if ( $user[ 'group_name' ] == Configure::read( 'group.ap' ) ) { //AP (with overrides)
            ...
            if ( $this->controller->Acl->check( array( 'model' => 'Users', 'foreign_key' => $id ), $this->controller->base . "add" ) ) {
                //item add form
                array_push( $m_items, [ 'xtype' => 'frmAttributeConvert' ]);
                array_push( $m_items, $this->btnAdd);
            }
        }
        if ( !empty( $m_items ) ) {
            $menu = [ 'xtype' => 'buttongroup', 'title' => $title,
    ...
    private function _fetchExtrasHomeServerPools( $title='' ) {
        $user = $this->user;
        ...
        $hs_btn = [
                    ...
                    'scale'     => $this->scale,
                    'itemId'    => 'home_servers',
                    'tooltip'   => __('home servers'),
                    'ui'        => $this->btnUiProfComp
                ];

        //Admin => all power
        if($user['group_name'] == Configure::read('group.admin')){  //Admin
            array_push($m_items,$hs_btn);
        }
        if($user['group_name'] == Configure::read('group.ap') || //AP (with overrides)
           $user['group_name'] == Configure::read('group.sm')){  //SM (with overrides)
            $id  = $user['id'];
            if($this->controller->Acl->check(['model' => 'Users', 'foreign_key' => $id], 'Access Providers/Controllers/HomeServers/index')){
                array_push($m_items,$hs_btn);
            }
        }
        if( !empty($m_items) ){
    ...
    private function _fetchIdpsExtras() {
        $user = $this->user;
        ...
        $sync_btn = [
                    ...
                    'scale'     => $this->scale,
                    'itemId'    => 'synchronize',
                    'tooltip'   => __('synchronize'),
                    'ui'        => $this->btnUiRedirect
                ];
        //SubGroups
        $sg_btn = [
                    'xtype'     => 'button',
                    'glyph'     => Configure::read('icnFolder'),
                    'scale'     => $this->scale,
                    'itemId'    => 'subGroups',
                    'tooltip'   => __('organization'),
                    'ui'        => $this->btnUiProfComp
                ];
        //Admin => all power
        if($user['group_name'] == Configure::read('group.admin')){  //Admin
            array_push($m_items,$sync_btn);
            array_push($m_items,$sg_btn);

        }
        if($user['group_name'] == Configure::read('group.ap') || //AP (with overrides)
           $user['group_name'] == Configure::read('group.sm')){  //SM (with overrides)
            $id  = $user['id'];
            if($this->controller->Acl->check(['model' => 'Users', 'foreign_key' => $id], 'Access Providers/Controllers/GoogleWsLinks/synchronize')){
                array_push($m_items,$sync_btn);
            }
            if($this->controller->Acl->check(['model' => 'Users', 'foreign_key' => $id], 'Access Providers/Controllers/SubGroups/index')){
                array_push($m_items,$sg_btn);
            }
        }
        if( !empty($m_items) ){
    ...
    private function _fetchSubGroups() {
        $user = $this->user;
    ...
    private function _fetchBasicSubGroups($action='disabled',$with_reload_timer=true){
    ...
    private function _fetchSubGroupsExtras() {
        $user = $this->user;
    ...
    private function _fetchBasicStaInfos($action='disabled',$with_reload_timer=true){
    ...
    private function _fetchSoftEtherVpnInstancesExtras() {
        $user = $this->user;
        ...
        if($user['group_name'] == Configure::read('group.ap')){ //AP (with overrides)
            $id  = $user['id'];
            if($this->controller->Acl->check(['model' => 'Users', 'foreign_key' => $id], 'Access Providers/Controllers/SoftEtherVpnInstances/synchronize')){
                array_push($m_items,$sync_btn);
            }
        }
        if( !empty($m_items) ){
    ...
    private function _fetchhBasicSoftEtherLocalBridges($action='disabled',$with_reload_timer=true){
    ...
    private function _fetchhBasicSoftEtherWireguardPublicKeys($action='disabled',$with_reload_timer=true){
        ...
        if($user['group_name'] == Configure::read('group.ap')){ //AP (with overrides)
            ...
            array_push($action_group,$reload);
            //Add
            if($this->controller->Acl->check(array('model' => 'Users', 'foreign_key' => $id), $this->controller->base."addPublicKey")){
                array_push($action_group,$this->btnAdd);
            }
            //Delete
            if($this->controller->Acl->check(array('model' => 'Users', 'foreign_key' => $id), $this->controller->base.'deletePublicKey')){
                array_push($action_group,$this->btnDelete);
            }
    ...
    private function _fetchhBasicSoftEtherInterfaces($action='disabled',$with_reload_timer=true){
        ...
        if($user['group_name'] == Configure::read('group.ap')){ //AP (with overrides)
            ...
            array_push($action_group,$reload);
            //Add
            if($this->controller->Acl->check(array('model' => 'Users', 'foreign_key' => $id), $this->controller->base."addInterfaces")){
                array_push($action_group,$this->btnAdd);
            }
            //Delete
            if($this->controller->Acl->check(array('model' => 'Users', 'foreign_key' => $id), $this->controller->base.'deleteInterfaces')){
                array_push($action_group,$this->btnDelete);
            }
