...
class AccessProvidersController extends AppController{
    ...
    public function indexTreeGrid(){
        ...
        $ap_name    = Configure::read('group.ap'); // Not used.
        $query      = $this->{$this->main_model}->find()->contain(['UserNotes','Groups']);
        ...
        foreach($q_r as $i){
            ...
            //Append group name
            $row['group_name'] = __($i->group->name);
    ...
    public function indexTree(){
        ...
        $sm_name    = Configure::read('group.sm');
        $query      = $this->{$this->main_model}->find();
        $q_r        = $query->where([$this->main_model.'.parent_id' => $id])->all();
    ...
    public function add(){
        ...
        if (!is_null($group_id) && isset($group_id)) {
            $ap_name    = Configure::read('group.ap');
            $sm_name    = Configure::read('group.sm');

            $q_r        = $this->Groups->find()->where(['Groups.id' => $group_id])->first();
            if (is_null($q_r) || ($q_r->name != $ap_name && $q_r->name != $sm_name)) {
                $group_id = null;
    ...
    public function edit(){
        ...
        if (!is_null($group_id) && isset($group_id)) {
            ...
            $q_r        = $this->Groups->find()->where(['Groups.id' => $group_id])->first();
            if (is_null($q_r) || ($q_r->name != $ap_name && $q_r->name != $sm_name)) {
                $this->request->data['group_id'] = null;
            }
        }
