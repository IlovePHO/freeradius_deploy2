...
class DashboardController extends AppController{
    ...
    public function authenticate(){
        ...
        if ($this->request->is('post')) {
            ...
            if ($user){
                ...
                $enable_rolling_token = Configure::read('Dashboard.enable_rolling_token');
                if (!is_null($enable_rolling_token) && $enable_rolling_token) {
    ...
    public function i18n(){
     ...
     public function utilitiesItems(){
        ...
        if ($group == Configure::read('group.admin') ||
            $group == Configure::read('group.ap')) {
     ...
     public function settingsView(){
        ...
        if($e_user->country_id && $e_user->language_id){
            $language = $e_user->country_id.'_'.$e_user->language_id;
        }
     ...
     public function settingsSubmit(){
        ...
        if(isset($this->request->data['timezone_id'])){
            ...
            if($this->request->getData('language')){
                ...
                $country_id         = $country_language[0];
                $language_id        = $country_language[1];

                if($e_user->country_id != $country_id || $e_user->language_id != $language_id ){
                    $languages = TableRegistry::get('languages');
                    $language = $languages->find()->where(['id' => $language_id])->first();
                    if(!empty($language)){
                      // Dashboard user's Language
                      $lang_code = $language->iso_code;
                    }
                }
            }
            if($e_user){
                $this->{'Users'}->patchEntity($e_user,[
                  'timezone_id' => $this->request->data['timezone_id'],
                  'email' =>$email,
        ...
        if($lang_code){
            // User's language
            $white_label['language']     = $lang_code;
            $white_label['sel_language'] = $country_id.'_'.$language_id;
        }
        $this->set(array(
            'success'   => true,
            'data'      => $white_label,
    ...
    private function _get_user_detail($user,$auto_compact=false){
        ...
        $lang_code = I18n::getLocale();
        $languages = TableRegistry::get('languages');
        $language = $languages->find()->where(['id' => $user->language_id])->first();
        if(!empty($language)){
          // Dashboard user's Language
          I18n::setLocale($language->iso_code);
          $lang_code = $language->iso_code;
        }
        ...
        if ($group == Configure::read('group.sm')){  //Or SM
            $cls = 'access_provider';
            $tabs= $this->_build_ap_tabs($id, $display, false);  //We DO care for rights here!
            $show_wizard = false;
            $show_unknown_nodes = false;
        }
        ...
        return [
            ...
            // User's language
            'language'     => $lang_code,
            'sel_language' => $user->country_id.'_'.$user->language_id,
        ];
    }
     ...
     private function _build_admin_tabs($user_id,$style = 'take_setting'){
        ...
        array_push($tabs, array(
                ...
                'items'   => array(
                   ...
                   //proxy-decision-conditions
                   array(
                        'title'   => __('Proxy decision conditions'),
                       // 'glyph'   => Configure::read('icnNas'),
                        'id'      => 'cProxyDecisionConditions',
                        'layout'  => 'fit'
                    ),
                    //attribute-converts
                    array(
                        'title'   => __('Attribute converts'),
                       // 'glyph'   => Configure::read('icnNas'),
                        'id'      => 'cAttributeConverts',
                        'layout'  => 'fit'
                    ),
                )
            )
        ...
        */
        ...
        array_push($tabs, array(
                ...
                'items'   => array(
                    [
                        'title'   => __('VPN instances'),
                        'glyph'   => Configure::read('icnNas'),
                        'id'      => 'cSoftEtherVpnInstances',
                        'layout'  => 'fit',
                        'tabConfig'=> [
                            'ui' => $this->tabUIOne
                        ]
                    ...
                    [
                        'title'   => __('Virtual Hubs'),
                        'id'      => 'cSoftEtherVirtualHubs',
                        'layout'  => 'fit',
                    ],
                    [
                        'title'   => __('Local Bridges'),
                        'id'      => 'cSoftEtherLocalBridges',
                        'layout'  => 'fit',
                    ],
                    [
                        'title'   => __('L2TP/IPsec'),
                    ...
                    [
                        'title'   => __('WireGuard'),
                        'id'      => 'cSoftEtherWireguard',
                        'layout'  => 'fit',
                    ],
                    [
                        'title'   => __('Network Bridges'),
                        'id'      => 'cSoftEtherNetworkBridges',
                        'layout'  => 'fit',
                    ],
    ...
    private function _build_ap_tabs($id, $style = 'take_setting', $enable_overview = true){
        $tabs   = array();
        $user_id = $id;
        ...
        if($this->Acl->check(array('model' => 'Users', 'foreign_key' => $id), $base."Idps/index")){
            array_push($admin_items, array(
                        'title'   => __('Institution'),
                        //'glyph'   => Configure::read('icnRealm'),
                        'id'      => 'cIdps',
        ...
        if($this->Acl->check(array('model' => 'Users', 'foreign_key' => $id), $base."ExternalApiKeys/index")){
            array_push($admin_items, array(
                        'title'   => __('API key'),
                        'id'      => 'cExternalApiKeys',
                        'layout'  => 'fit',
        ...
        if($this->Acl->check(array('model' => 'Users', 'foreign_key' => $id), $base."EncodingSchemes/index")){
            array_push($admin_items, array(
                        'title'   => __('HMAC key'),
                        'id'      => 'cEncodingSchemes',
                        'layout'  => 'fit',
       ...
       if($this->Acl->check(array('model' => 'Users', 'foreign_key' => $id), $base."StaConfigs/index")){
            array_push($admin_items, array(
                        'title'   => __('STA config'),
                        'id'      => 'cStaConfigs',
                        'layout'  => 'fit',
        ...
        */
        ...
        if($this->Acl->check(array('model' => 'Users', 'foreign_key' => $id), $base."StaInfos/index")){
            array_push($users_items, array(
                    'title'     => __('STA infos'),
                    //'glyph'     => Configure::read('icnDevice'),
                    'id'        => 'cStaInfos',
      ...
      /*  if($this->Acl->check(array('model' => 'Users', 'foreign_key' => $id), $base."ProfileComponents/index")){
        ...
        if($this->Acl->check(array('model' => 'Users', 'foreign_key' => $id), $base."Proxies/index")){
            array_push($radius_items, array(
                    'title'   => __('Proxies'),
                   // 'glyph'   => Configure::read('icnNas'),
                    'id'      => 'cProxies',
        ...
        //proxy-decision-conditions
        if($this->Acl->check(array('model' => 'Users', 'foreign_key' => $id), $base."AttributeConverts/index")){
            array_push($radius_items, array(
                        'title'   => __('Proxy decision conditions'),
                       // 'glyph'   => Configure::read('icnNas'),
                        'id'      => 'cProxyDecisionConditions',
    ...
    /*    if($this->Acl->check(array('model' => 'User', 'foreign_key' => $id), $base."Analytics/index")){
      ...
      /*
        ...
        */
        ...
        //VPN
        $vpn_items = [];
        if($this->Acl->check(array('model' => 'Users', 'foreign_key' => $id), $base."SoftEtherVpnInstances/index")){
            array_push($vpn_items, [
                    'title'   => __('VPN instances'),
                    'glyph'   => Configure::read('icnNas'),
                    'id'      => 'cSoftEtherVpnInstances',
                    'layout'  => 'fit',
        ...
        if($this->Acl->check(array('model' => 'Users', 'foreign_key' => $id), $base."SoftEtherVirtualHubs/index")){
            array_push($vpn_items, [
                        'title'   => __('Virtual Hubs'),
                        'id'      => 'cSoftEtherVirtualHubs',
                        'layout'  => 'fit',
        ...
        if($this->Acl->check(array('model' => 'Users', 'foreign_key' => $id), $base."SoftEtherLocalBridges/index")){
            array_push($vpn_items, [
                        'title'   => __('Local Bridges'),
                        'id'      => 'cSoftEtherLocalBridges',
                        'layout'  => 'fit',
        ...
        if($this->Acl->check(array('model' => 'Users', 'foreign_key' => $id), $base."SoftEtherL2tp/view")){
            array_push($vpn_items, [
                        'title'   => __('L2TP/IPsec'),
                        'id'      => 'cSoftEtherL2tp',
                        'layout'  => 'fit',
        ...
        if($this->Acl->check(array('model' => 'Users', 'foreign_key' => $id), $base."SoftEtherWireguard/view")){
            array_push($vpn_items, [
                        'title'   => __('WireGuard'),
                        'id'      => 'cSoftEtherWireguard',
                        'layout'  => 'fit',
        ...
        if($this->Acl->check(array('model' => 'Users', 'foreign_key' => $id), $base."SoftEtherNetworkBridges/index")){
            array_push($vpn_items, [
                        'title'   => __('Network Bridges'),
                        'id'      => 'cSoftEtherNetworkBridges',
                        'layout'  => 'fit',
