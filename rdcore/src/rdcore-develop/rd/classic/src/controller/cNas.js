Ext.define('Rd.controller.cNas', {
    ...
    views:  [
        ...
        'nas.cmbNasTypes', 
        'nas.pnlNasMaps',
        'components.cmbNas', 'nas.winMapNasAdd', 'nas.pnlNasPhoto', 
    ...
    config: {
        ...
        urlUploadPhoto:     '/cake3/rd_cake/nas/upload_photo.json',
    ...
    refs: [
        ...
        {  ref: 'pnlNasMaps',  selector:   'pnlNasMaps'},
    ...
    init: function() {
        ...
        me.control({
            ...
            //map preferences
            '#winMapPreferencesNas #snapshot': {
                click:      me.mapPreferencesSnapshot
            },
            '#winMapPreferencesNas #save': {
    ...
    gridRealmsForNasOwnerActivate: function(tab){
        ...
        grid.getStore().getProxy().setExtraParam('owner_id',me.user_id);
    ...
    mapPreferences: function(button){
        var me = this;
        if(!Ext.WindowManager.get('winMapPreferences')){
            var w = Ext.widget('winMapPreferences',{id:'winMapPreferencesNas'});
            w.show();
    ...
    mapNasAddSubmit: function(button){
        ...
        var map_tab   = tab_panel.down('#nasMapTab');
        if(map_tab != null){
            var nasMap = map_tab.down('pnlNasMaps');
            nasMap.addMarker(record.data,'new');
        }
    },
        ...
        form.submit({
            ...
            success: function(form, action) {
                if(action.result.success){ 
                    var new_img = action.result.data.photo_file_name;
    ...
    //____ MAP ____
    /*GoogleMaps → Leaflet + OpenStreetMap*/
                ...
                if(jsonData.success){     
                  // console.log(jsonData);
                    ...
                    tp.add({ 
                        ...
                        items: {
                                xtype: 'pnlNasMaps',
                                mapOptions: {zoom: jsonData.data.zoom, mapType:jsonData.data.type },
    ...
    mapPreferencesSnapshot: function(button){
        ...
        /*GoogleMaps → Leaflet + OpenStreetMap
            "type" is invalid
        */
        var pnl = me.getPnlNasMaps();
        var info = pnl.getMapPreferences();
        console.log(info);
        form.down('#lat').setValue(info.center.lat);
        form.down('#lng').setValue(info.center.lng);
        form.down('#zoom').setValue(info.zoom);
        // form.down('#type').setValue(info.type);
    },
