Ext.define('Rd.controller.cAcos', {
    ...
    actionIndex: function(pnl){
        ...
        pnl.add({
            ...
            items   : [
                  ...
                  { 'title' : i18n('sSite_Manager_Rights'),'xtype':'treeSmRights'},
            ]
        });
        me.populated = true;   
    },
    views:  ['acos.treeAco','acos.winAcoAdd','acos.winAcoEdit','acos.treeApRights','acos.treeSmRights'],
    stores: ['sAcos','sApRights','sSmRights'],
    models: ['mAco','mApRight','mSmRight'],
    ...
    refs: [
         ...
         {  ref:    'treeSmRights', selector:   'treeSmRights'}
    ],
    init: function() {
        me = this;
        if (me.inited) {
        ...
        me.control({
            ...
            'treeSmRights advCheckColumn': {
                checkchange: me.smRightChange
            },
            '#tabAcos treeSmRights'   : {
                itemclick:  me.smRightsGridClick
            },
            'treeSmRights #reload': {
                click:      me.smRightsReload
            },
            'treeSmRights #expand': {
                click:      me.smRightsExpand
            }
        });;
    },
    appClose:   function(){
    ...
    smRightChange: function(){
        var me = this;
        me.getTreeSmRights().getStore().sync({
            success: function(batch,options){
                Ext.ux.Toaster.msg(
                    i18n('sItem_updated'),
                    i18n('sItem_updated_fine'),
    ...
    smRightsGridClick:  function(grid, record, item, index, event){
        var me = this;
        me.smRightsSelectedRecord = record;
    },
    smRightsReload: function(){
        var me =this;
        me.getStore('sSmRights').load();
    },
    smRightsExpand: function(){
        var me = this;
        //See if there are anything selected... if not, inform the user
        var sel_count = me.getTreeSmRights().getSelectionModel().getCount();
        if(sel_count == 0){
            Ext.ux.Toaster.msg(
                        i18n('sSelect_a_node'),
                        i18n('sFirst_select_a_node_to_expand'),
            ...
            me.getTreeSmRights().expandNode(me.smRightsSelectedRecord,true);
        }
    }
