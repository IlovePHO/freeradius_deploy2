Ext.define('Rd.controller.cDynamicClients', {
    ...
    actionIndex: function(pnl){
        ...
        var me = this;
        me.ui   = Rd.config.tabDynamicClients; //This is set in the config file 
        if (me.populated) {
            return; 
        }     
        pnl.add({
    ...
    models: ['mAccessProviderTree', 'mDynamicClient', 'mRealmForDynamicClientOwner', 'mUnknownDynamicClient','mDynamicClientState','mUserStat','mAlive'],
    selectedRecord: null,
    config: {
    ...
    init: function() {
        ...
        me.control({
            ...
            '#winMapPreferencesDc #snapshot': {
                click:      me.mapPreferencesSnapshot
            },
            '#winMapPreferencesDc #save': {
    ...
    mapPreferences: function(button){
        var me = this;
        if(!Ext.WindowManager.get('winMapPreferencesDc')){
            var w = Ext.widget('winMapPreferences',{id:'winMapPreferencesDc'});
            w.show();
