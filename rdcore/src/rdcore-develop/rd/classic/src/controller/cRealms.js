Ext.define('Rd.controller.cRealms', {
    ...
    config: {
        ...
        urlProxiesAdd:      '/cake3/rd_cake/proxies-realms/add.json',
        urlProxiesDelete:   '/cake3/rd_cake/proxies-realms/delete.json'
    },
    refs: [
    ...
    init: function() {
        ...
        me.control({
            ...
            'gridRealms #ProxyOperation':{
                click: me.ProxyOperation
            },
            'winProxiesRealms #save': {
                click:  me.ProxyOperationSubmit
            },
        });;
    },
    ...
    proxies: function(button){
        var me = this;
        // Go to cProxies tab
        var tpDash = button.up('tpDashboard');
        var tp =tpDash.down('#cProxies').up('tabpanel');
        if(tp){
            tpDash.setActiveTab(tp);
            tp.setActiveTab('cProxies');
        }
    },
    ProxyOperation: function(button){
        var me      = this;    
        //Find out if there was something selected
        var sel_count = me.getGrid().getSelectionModel().getCount();
        if(sel_count == 0){
            ...
            if(sel_count > 1){
                ...
                if(!Ext.WindowManager.get('winProxiesRealms'+sr.getId())){
                    var w = Ext.widget('winProxiesRealms',
                        {
                            id          : 'winProxiesRealms'+sr.getId(),
                            realm_id   : sr.getId(),
                            realm_name : sr.get('name')
                        });
                    w.show();
                }
            }
    ...
    ProxyOperationSubmit:function(button){
        var me      = this;
        var win     = button.up('window');
        var form    = win.down('form');
        var cmb     = form.down('#proxy_id');
        var rbg     = form.down('#action_rd');
        ...
        if((rbg.getValue().rb == 'add')||(rbg.getValue().rb == 'remove')){
            ...
            //Checks passed fine...      
            var postUrl = (rbg.getValue().rb == 'add')? me.getUrlProxiesAdd() : me.getUrlProxiesDelete() ;
    ...
    subGroups: function(button){
        ...
        if(me.getGrid().getSelectionModel().getCount() == 0){
            ...
            Ext.each(me.getGrid().getSelectionModel().getSelection(), function(sr,index){
                ...
                me.application.runAction('cSubGroups','Index',tp,{name:name,realm_id:id});
            });
        }
    },
});
